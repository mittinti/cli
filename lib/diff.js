...
class Diff extends BaseCommand {
  ...
  argsError () {
    const msg = 'Needs multiple arguments to compare when not running from a project dir.'
    return new Error(`${msg}\n\nUsage:\n${this.usage}`)
  }

  get where () {
    return resolve(this.npm.dir, '..')
  }
  ...
  async defaultSpec () {
    let pkgName
    try {
      pkgName = await readPackageName(this.where)
    } catch (e) {
      npmlog.verbose('diff', 'could not read project dir package.json')
    }

    if (!pkgName) {
      throw this.argsError()
    }
  ...
  async transformSingleSpec (a) {
    ...
    const missingPackageJson = this.argsError()

    const specSelf = () => {
      if (noPackageJson)
        throw missingPackageJson

      return `file:${this.where}`
    }
